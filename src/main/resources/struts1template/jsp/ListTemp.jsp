...
<html>
	<head>
		<title><bean:message bundle="&{appname}Res" key="&{entityname}.list.title" /></title>
	...
	<body>
		...
		<fieldset
			...
			<legend>
				 <bean:message key="label.list.filter" />
			</legend>
			<html:form action="/&{appname}/&{entityname}.do" style="margin-top:0;margin-bottom:5">
				<table cellpadding="0" cellspacing="0" align="center">
&{for-each:keyproperty}
					<tr  height="22">
						<td><bean:message bundle="&{appname}Res" key="&{entityname}.&{keyproperty}" />:</td>
						<td><html:text property="&{keyproperty}_SEARCH_" /> </td>
					</tr>	
&{end-for-each}
&{for-each:property}
					<tr height="22">
						<td><bean:message bundle="&{appname}Res" key="&{entityname}.&{property}" />:</td>
						<td><html:text property="&{property}_SEARCH_" /> </td>
					</tr>	
&{end-for-each}
					<tr>
						<td>
